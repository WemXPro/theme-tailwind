                            ...
                            @if($field['type'] == 'select')
                                <select class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    ...
                                    @foreach($field['options'] ?? [] as $key => $option)
                                        <option value="{{ $key }}"
                                                @if(in_array($key, (array) $package->data(Str::remove("[]", $field['key']), $field['default_value'] ?? ''))) selected @endif >{{ $option }}
                                        </option>
                                    @endforeach
                                </select>
                            @elseif($field['type'] == 'bool')
                                <label class="relative inline-flex items-center cursor-pointer mt-2">
                                    ...
                                    <input type="checkbox" name="{{ $field['key'] }}" value="{{$field['default_value'] ?? '0'}}" class="sr-only peer"
                                           @if($package->data($field['key'], $field['default_value'] ?? '0')) checked
                                           @endif
                                           @if(isset($field['disabled']) AND $field['disabled']) disabled @endif>
