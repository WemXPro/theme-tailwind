...
@section('container')
    ...
    <ul class="mb-5 mt-8 grid w-full gap-6 md:grid-cols-3">
        @foreach ($package->prices->where('is_active', true) as $price)
            <li>
                <input type="radio" id="price-radio-{{ $price->id }}" name="price_id" value="{{ $price->id }}" class="peer hidden"
                    required @if ($price->id == request()->input('price', $package->prices->first()->id)) checked @endif>
                <label for="price-radio-{{ $price->id }}"
                    ...
                    <div class="block">
                        <div class="w-full text-lg font-semibold">
                            {{ currency('symbol') }}{{ number_format($price->renewal_price, 2) }} / {{ $price->periodToHuman() }}
                        </div>
                        <div class="w-full">
                            ...
                            @isset($price->data['badge'])
                                <span
                                    class="bg-primary-100 text-primary-800 dark:bg-primary-900 dark:text-primary-300 inline-block rounded px-2.5 py-0.5 text-xs font-medium">
                                    {{ $price->data['badge'] }}
                                </span>
                            @endisset
                        </div>
    ...
    @if ($package->require_domain)
        <div class="relative mb-6 mt-8 rounded-lg bg-white p-4 shadow dark:bg-gray-800 sm:p-5">
            <div class="custom-note">
                ...
                <label for="helper-text" class="mb-2 block text-sm font-medium text-gray-900 dark:text-white">{!! __('client.domain') !!}</label>
                <input type="text" id="helper-text" name="domain" value="{{ old('domain') }}" aria-describedby="helper-text-explanation"
                        ...
                        <div class="@isset($field['col']) {{ $field['col'] }} @else w-1/2 p-2 @endisset"
                            ...
                            @if ($field['type'] == 'select')
                                <select
                                    ...
                                    @if (isset($field['disabled']) and $field['disabled']) disabled @endif @if (isset($field['multiple']) and $field['multiple']) multiple @endif>
                            ...
                            @elseif($field['type'] == 'bool')
                                <label class="relative mt-2 inline-flex cursor-pointer items-center">
                                    ...
                                    <input type="checkbox" name="{{ $field['key'] }}" value="1" class="peer sr-only"
                                        @if (getValueByKey($field['key'], $package->data, $field['default_value'] ?? '0')) checked @endif @if (isset($field['disabled']) and $field['disabled']) disabled @endif>
                                    <div
                            ...
                            @else
                                <input
                                    ...
                                    @if (in_array('required', $field['rules'])) required="" @endif @if (isset($field['disabled']) and $field['disabled']) disabled @endif>
                            @endif
    ...
    <div class="relative mt-8 rounded-lg bg-white p-4 shadow dark:bg-gray-800 sm:p-5">
        ...
        <select
            ...
            @foreach (App\Models\Gateways\Gateway::getActive('subscription') as $gateway)
                <option @if ($gateway->default) selected @endif data-gateway-type="subscription" value="{{ $gateway->id }}">
                    {{ $gateway->name }}
        ...
        <div class="relative mt-8 rounded-lg bg-white p-4 shadow dark:bg-gray-800 sm:p-5" id="tax-card">
            ...
            <select
                ...
                @foreach (config('utils.countries') as $key => $country)
                    <option value="{{ $key }}" @if (request()->header('cf-ipcountry', auth()->user()->address->country ?? null) == $key) selected @endif>{{ $country }}</option>
                @endforeach
            </select>
        </div>
    @endif
    ...
    <div class="w-full pl-4 pl-4 pl-4 pr-4 pr-4 pr-4 sm:w-1/2 md:w-1/3 lg:w-1/3">
        <div class="sticky left-0 top-8 max-w-sm rounded-lg border-gray-200 bg-white p-6 shadow dark:bg-gray-800">
            ...
            <p class="mb-4 flex justify-between text-sm font-normal text-gray-700 dark:text-gray-400">
                ...
                <span>
                    {{ currency('symbol') }}<span id="recurring">{{ number_format($package->prices->first()->renewal_price, 2) }}</span>
                </span>
            </p>
            ...
            <p class="mb-4 flex justify-between text-sm font-normal text-gray-700 dark:text-gray-400">
                ...
                <span>
                    {{ currency('symbol') }}<span id="setup_fee">{{ number_format($package->prices->first()->setup_fee, 2) }}</span>
                </span>
            </p>
                ...
                <div class="mb-4 flex items-start">
                    ...
                    <div class="ml-3 text-sm">
                        <label for="terms" class="font-medium text-gray-900 dark:text-white">
                            {!! __('client.i_accept_the') !!}<a class="ml-1 text-blue-700 hover:underline dark:text-blue-500"
