<div class="flex items-center justify-between lg:order-2">
    ...
    <button id="dropdownPalleteButton" data-dropdown-toggle="dropdownPallete" aria-label="dropdownPallete"
        ...
        </svg><span class="bg-primary-600 dark:bg-primary-400 ml-2 h-4 w-6 rounded"></span><span
    ...
    <button data-tooltip-target="tooltip-dark" type="button" onclick="toggleDarkmode()" aria-label="{{ __('client.toggle_darkmode') }}"
        class="mr-1 inline-flex items-center rounded-lg p-2 text-sm font-medium text-gray-500 hover:bg-gray-50 focus:outline-none focus:ring-4 focus:ring-gray-300 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-800">
    ...
    @auth
        ...
        <button type="button" data-dropdown-toggle="notification-dropdown"
            ...
            @if (auth()->user()->notifications()->where('read_at', '=', null)->exists())
                <div class="relative flex" style="position: absolute">
                    <div class="relative left-3 inline-flex h-3 w-3 rounded-full border-2 border-white bg-red-500 dark:border-gray-900"
            ...
            <div class="grid">
                @foreach (auth()->user()->notifications()->latest()->paginate(5) as $notification)
                    <a @isset($notification->button_url) href="{{ $notification->button_url }}" @else href="#" @endisset
                        ...
                        <span class="text-primary-800 dark:text-primary-400 inline-flex items-center rounded-full p-1.5 text-sm font-semibold">
                            <div class="text-3xl">
                        ...
                        <div class="w-full pl-3">
                            <div class="mb-1.5 text-sm font-normal text-gray-500 dark:text-gray-400">{{ $notification->message }}</div>
                            <div class="text-xs font-medium text-gray-700 dark:text-gray-400">{{ $notification->created_at->diffForHumans() }}
                            </div>
                        </div>
                    </a>
                @endforeach
        ...
        <div class="z-50 my-4 hidden max-w-sm list-none divide-y divide-gray-100 overflow-hidden rounded bg-white text-base shadow-lg dark:divide-gray-600 dark:bg-gray-700"
            ...
            <div class="grid grid-cols-3 gap-4 p-4">
                ...
                <a href="{{ route('invoices') }}" class="group block rounded-lg p-4 text-center hover:bg-gray-100 dark:hover:bg-gray-600">
                    <div
                        class="mx-auto mb-1 text-3xl text-gray-400 group-hover:text-gray-500 dark:text-gray-400 dark:group-hover:text-gray-400">
        ...
        <div class="z-50 my-4 hidden w-56 list-none divide-y divide-gray-100 rounded bg-white text-base shadow dark:divide-gray-600 dark:bg-gray-700"
            ...
            <div class="px-4 py-3">
                ...
                <span class="block truncate text-sm font-light text-gray-500 dark:text-gray-400">{{ auth()->user()->email }}</span>
            </div>
