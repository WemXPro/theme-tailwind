...
<div id="drawer-example"
    ...
    <form action="{{ route('balance.add') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <div class="relative mb-6 mt-6">
                <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                    <svg class="w-5 h-5 text-gray-500 dark:text-gray-400" width="24" height="24"
                ...
                <input type="number" id="amount" name="amount"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5  dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    value="5">
            </div>
        ...
        <div class="">
            <button type="submit"
                class="w-full focus:outline-none text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">{!! __('client.pay_now') !!}</button>
        </div>
    </form>
</div>
<!-- drawer component -->
...
<div data-popover id="popover-user-profile" role="tooltip"
    ...
    <div class="p-3">
        <div class="flex items-center justify-between mb-2">
            ...
            <div>
                <button type="button"
                    class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-xs px-3 py-1.5 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">{{ __('client.follow') }}</button>
            </div>
        </div>
        ...
        <div class="sm:flex sm:items-center sm:justify-between">
            <span class="text-sm text-gray-500 sm:text-center dark:text-gray-400">
                ...
                <a href="https://wemx.net" target="_blank"
                    class="hover:underline text-primary-600 dark:text-primary-500">WemX™</a>.
            </span>
            <span class="text-sm text-gray-500 sm:text-center dark:text-gray-400">
                © {{ date('Y') }}
...
@endif
<script>
    function toggleDescription(categoryId, element) {
        var description = document.getElementById('categoryDescription_' + categoryId);
        var isExpanded = description.classList.contains('expanded');

        if (isExpanded) {
            description.classList.remove('expanded');
            description.innerHTML = description.getAttribute('data-truncated-description');
            element.innerText = "{{ __('Read More') }}";
        } else {
            // Collapse any expanded descriptions
            var expandedDescriptions = document.querySelectorAll('.expanded');
            expandedDescriptions.forEach(function(expandedDescription) {
                expandedDescription.classList.remove('expanded');
                expandedDescription.innerHTML = expandedDescription.getAttribute('data-truncated-description');
                var categoryId = expandedDescription.getAttribute('data-category-id');
                document.getElementById('readMoreLink_' + categoryId).innerText = "{{ __('Read More') }}";
            });

            // Expand the clicked description
            description.classList.add('expanded');
            description.innerHTML = description.getAttribute('data-full-description');
            element.innerText = "{{ __('Read Less') }}";
        }
    }
</script>
