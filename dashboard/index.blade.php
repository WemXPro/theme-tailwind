                        ...
                        <div
                            ...
                            <div class="flex w-full items-center space-x-3">
                                ...
                                <div class="font-medium text-gray-400">
                                    {{ count(auth()->user()->orders()->where('status', Cookie::get('filter_orders', 'active'))->orderBy('status', 'asc')->orderBy('created_at', 'desc')->paginate(10)) }}
                                    {!! __('client.results') !!}</div>
                                ...
                                @if (count(auth()->user()->orders()->where('status', Cookie::get('filter_orders', 'active'))->orderBy('status', 'asc')->orderBy('created_at', 'desc')->paginate(10)) > 10)
                                    ...
                                    <div id="results-tooltip" role="tooltip"
                                        ...
                                        {!! __('client.showing', [
                                            'count' => '1-10',
                                            'all' => count(
                                                auth()->user()->orders()->where('status', Cookie::get('filter_orders', 'active'))->orderBy('status', 'asc')->orderBy('created_at', 'desc')->paginate(10),
                                            ),
                                        ]) !!}
                            ...
                            <div class="flex w-full flex-row items-center justify-end space-x-3">
                                <button id="filterDropdownButton" data-dropdown-toggle="filterDropdown"
                                    ...
                                    type="button">
                                    <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="mr-2 h-4 w-4 text-gray-400"
                                    ...
                                    {{ __('client.filter') }}
                                    <svg class="-mr-1 ml-1.5 h-5 w-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"
                                        aria-hidden="true">
                                    ...
                                    <ul class="space-y-2 text-sm" aria-labelledby="filterDropdownButton">
                                        @foreach (auth()->user()->orders()->distinct()->pluck('status') as $status)
                                            <li class="flex items-center"
                                                ...
                                                <input id="{{ $status }}" @if (Cookie::get('filter_orders', 'active') == $status) checked @endif type="checkbox"
                                                    value="{{ $status }}"
                                ...
                                <tbody data-accordion="table-column">
                                    @foreach (auth()->user()->orders()->where('status', Cookie::get('filter_orders', 'active'))->orderBy('status', 'asc')->orderBy('created_at', 'desc')->paginate(10) as $order)
                                        <tr class="cursor-pointer border-b transition hover:bg-gray-200 dark:border-gray-700 dark:hover:bg-gray-700"
                                            id="table-column-header-0" data-accordion-target="#table-column-body-{{ $order->id }}"
                                            ...
                                            <th scope="row"
                                                ...
                                                <img class="mr-2 h-9 w-9 rounded"
                                                    src="{{ asset('storage/products/' . $order->package['icon']) }}" alt="">
                                                ...
                                                <div class="flex -space-x-4 rtl:space-x-reverse">
                                                    ...
                                                    @foreach ($order->members()->paginate(2) as $member)
                                                        @if ($member->user->avatar ?? false)
                                                            <img class="@if ($loop->last) z-10 @endif h-10 w-10 rounded-full border-2 border-white dark:border-gray-800"
                                        ...
                                        <tr class="hidden w-full flex-1 overflow-x-auto" id="table-column-body-{{ $order->id }}"
